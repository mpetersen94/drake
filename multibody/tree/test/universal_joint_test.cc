...
TEST_F(UniversalJointTest, SetVelocityAndAccelerationLimits) {
  ...
  EXPECT_THROW(mutable_joint_->set_velocity_limits(VectorX<double>(2),
                                                   VectorX<double>()),
               std::runtime_error);
  ...
  // Does not match num_velocities().
  EXPECT_THROW(mutable_joint_->set_acceleration_limits(VectorX<double>(2),
                                                       VectorX<double>()),
               std::runtime_error);
  EXPECT_THROW(mutable_joint_->set_acceleration_limits(VectorX<double>(),
               ...
               std::runtime_error);
  // Lower limit is larger than upper limit.
  EXPECT_THROW(mutable_joint_->set_acceleration_limits(Vector2d::Constant(2),
                                                       Vector2d::Constant(0)),
               std::runtime_error);
}
