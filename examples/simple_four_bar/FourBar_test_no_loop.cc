...
int DoMain() {
  ...
  const std::string urdf = "drake/examples/simple_four_bar/FourBar_sq_no_loop.urdf";
  {
    ...
    const int frame1 = tree->FindBodyIndex("base_link");
    const int frame2 = tree->FindBodyIndex("link_2");
    Vector3<double> point1(0, 0, 2);
    Vector3<double> point2(0, 0, 2);

    tree->addDistanceConstraint(frame1,point1,frame2,point2,2.0);
