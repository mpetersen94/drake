...
/// y-axis of frame M  respectively.
/// Mathematically, rotation `R_FM` is given in terms of angles θ₁, θ₂ by: <pre>
///   R_FM(q) = R_FI(θ₁) * R_IM(θ₂)
/// </pre>
/// where `R_FI(θ)` and `R_IM(θ)` correspond to the elemental rotations in
/// amount of θ about the x and y axes respectively.
...
class UniversalMobilizer final : public MobilizerImpl<T, 2, 2> {
 public:
  ...
  /// @param[in] context The context of the model this mobilizer belongs to.
  /// @returns angles_dot The rate of change of the two angles, θ̇₁, θ̇₂, returned
  ///                     as the vector [θ̇₁, θ̇₂].
  ...
  /// @param[in] context The context of the model this mobilizer belongs to.
  /// @param[in] angles_dot The desired rate of change, θ̇₁, θ̇₂, packed as the
  ///                       vector [θ̇₁, θ̇₂].
  /// @returns a constant reference to `this` mobilizer.
 ...
 protected:
  Eigen::Matrix<T, 3, 2> CalcHwMatrix(const systems::Context<T>& context) const;
