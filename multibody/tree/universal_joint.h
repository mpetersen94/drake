...
/// This joint models allows two bodies to rotate relative to one another with
...
class UniversalJoint final : public Joint<T> {
 public:
  ...
  /// angular rates for `this` joint (see get_angular_rates()).
  ...
  /// parameterized with angles θ₁, θ₂, corresponding to a rotation about the
  /// x-axis of parent frame F followed by a rotation about the y-axis of child
  /// frame M. Mathematically, rotation `R_FM` is given in terms of angles
  /// θ₁, θ₂ by: <pre>
  ///   R_FM(q) = R_My(θ₂) * R_Fx(θ₁)
  /// </pre>
  /// where `R_Fx(θ)` and `R_My(θ)` correspond to the elemental rotations by
  /// angle θ about the x-axis of parent frame F and y-axis of child frame M
  /// respectively.
  /// Zero θ₁, θ₂ angles corresponds to frames F and M being coincident.
  /// Angles θ₁, θ₂ are defined to be positive according to the right-hand-rule
